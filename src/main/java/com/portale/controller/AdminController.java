...
public class AdminController {
	...
	@RequestMapping(value = "/get/rooms/{page}/{items_per_page}", method = RequestMethod.GET, headers = "Accept=application/json")
	@ResponseBody
	public ResponseEntity<?> getRooms(@PathVariable("page") int page, @PathVariable("items_per_page") int items_per_page) {
		try {
			Data users = adminService.getRooms(page, items_per_page);
			return new ResponseEntity<>(users, HttpStatus.OK);
		} catch (ManagedException e) {
			return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
		} catch (Exception e) {
